...
public:
  ...
  PlayerHUD(const Player& player, const sf::Texture& hudTex, const sf::Font& usernameFont, const sf::Font& coinsFont, sf::Vector2f pos)
    : m_player(&player),
    m_sprite(hudTex),
    m_usernameText(usernameFont),
    m_chipsText(coinsFont)
  {
    ...
    m_chipsText.setCharacterSize(COINFONT_SIZE);
    m_chipsText.setFillColor(sf::Color::White);
