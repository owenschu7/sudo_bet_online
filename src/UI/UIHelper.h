...
inline void placeAllCards(std::vector<CardSprite>& cards, float baseY)
{
  ...
  for (size_t i = 0; i < cards.size(); ++i)
  {
    if (i < 3)
    {
      ...
      placeCardInHand(cards[i].getSFMLSprite(), playerStartX, baseY, i);
    }
    else
  {
      // Banker cards (indices 3, 4, 5)
      placeCardInHand(cards[i].getSFMLSprite(), bankerStartX, baseY, i - 3);
    }
  }
}
